...
/**
 * 采取RSA算法对文件压缩加密/解密解压缩 对象类
 * @author 丁腾飞
 * @date 2010/11/18
 * @version 1.0.0
 */
public class RSAZipEncrypt {
	...
	public static void encryptZip(String srcFile, String destfile,
		...
		// 获得私匙
		RSAZipEncrypt.privateKey = kp.getPrivate();
		// 获得公钥
		RSAZipEncrypt.publicKey = kp.getPublic();
		File f = new File(keyfile);
		f.createNewFile();
		FileOutputStream fos = new FileOutputStream(f);
		ObjectOutputStream dos = new ObjectOutputStream(fos);
		dos.writeObject(RSAZipEncrypt.publicKey);
	...
	public static void main(String args[]) throws Exception {
		...
		String srcPath = "C:\\sag\\license.config";
		
		// 压缩后的存放文件
		//String destZip = prop.getProperty("DEST_FILE");
		String destZip = "C:\\sag\\license.zip";
		
		// 压缩加密后的publickey
		//String keyfile = prop.getProperty("KEY_FILE");
		String keyfile = "C:\\sag\\keyfile.properties";
		
		RSAZipEncrypt.encryptZip(srcPath, destZip, keyfile);

		/*
		 * 解密 ZipEncrypt.decryptUnzip("e:/comXXX/comxxxx.zip", "d:/comxxx",
		 * ZipEncrypt .getKey("e:/comXXX/public.key"));
		 */
		//EncryptZip.decrypt("C:\\sag\\license.config", "C:\\sag\\", "sortec");
	}
}
