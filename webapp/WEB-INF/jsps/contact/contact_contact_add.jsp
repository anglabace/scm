...
function add_contact(bt_obj, form_name, action_name){
	...
	var options = {
		success:function(data)
		{
			...
			parent.$('#addContactCntctDialog').dialog('close');
		},
		error: function(){
			alert('error...');
			bt_obj.value = bt_val;
...
function validate_form()
{
	...
	if(tmp_str1 == '' && tmp_str2){
		...
		if(compareFlag == 1){
			alert("Please enter the correct Contact Date.");
			$('#contactDate').focus();
			return false;
		}
	}
...
<body>
<form method="post" action="contact_contact/save.action" id="save_contact_form">
<table width="660" border="0"  cellpadding="0" cellspacing="0" >
  ...
  <tr>
	...
	<div id="Phone" style="display:none;">
        ...
        <s:select name="callTimeFromAPM" cssStyle="width:59px;" list="amPmList" headerKey=" " headerValue="" value="callTimeFromAPM" />
  ...
  <tr>
    <td>
        <div align="center" style="margin-top:15px;">
          ...
          <input type="button" value="  Add  " onclick="add_contact(this, 'save_contact_form', 'contact_contact!save.action?sessContactNo=${sessContactNo}');" />
          <input type="button" value="Cancel" onclick="parent.$('#addContactCntctDialog').dialog('close');"  />
      	</div>     
      </td>
  </tr>
</table>
...
<script>
changeContent();
function init_check(contactAddrsCount, shipAddrsCount, billAddrsCount, soldAddrsCount)
{
	if(billAddrsCount > 0)
	{
...
var contactAddrsCount = "${contactAddrsCount}";
var shipAddrsCount = "${shipAddrsCount}";
var billAddrsCount = "${billAddrsCount}";
var soldAddrsCount =  "${soldAddrsCount}";
init_check(contactAddrsCount, shipAddrsCount, billAddrsCount, soldAddrsCount);
</script>
</body>
</html>
