...
<link href="${global_js_url}jquery/jquery.jgrowl.css" rel="stylesheet" type="text/css" />
...
<script src="${global_js_url}jquery/jquery.jgrowl_minimized.js" type="text/javascript"></script>
...
function addA(str){
	var sequenceId = parent.$('#seqSelectDlg').data("sequenceId");
	var oldStr = $(window.parent.document).find("#moreDetailIframe").contents().find('#'+sequenceId).val();
	oldStr += str;
	$(window.parent.document).find("#moreDetailIframe").contents().find('#'+sequenceId).val(oldStr);
	$.jGrowl(str + "is added!.", { 
		sticky: false,
		theme: 'iphone'
	});
...
<div id="Notification"/>
  ...
  <tr>
    <td colspan="9">Please click <img src="${global_image_url}append.gif" /> button to add you amino acid code. <br />
      After you   have finished your selection, click 'OK' to add them to your   order.</td>
  </tr>
</table>
        ...
        <td valign="top" bgcolor="#FFFFFF"><table width="100%">
          <tbody id="leftTable">
            ...
            <s:if test="#sts.odd == true"><tr>
              ...
              <img src="${global_image_url}append.gif" style="cursor: pointer" name="a0" border="0" id="a0" onclick="addA('${code3}')" /></c:if></td>
              <td>${dcode}</td><td><c:if test="${! empty dcode}">
              <img src="${global_image_url}append.gif" style="cursor: pointer" name="d0" border="0" id="d0" onclick="addA('${dcode}')" /></c:if></td>
            </tr>
            </s:if>
            </s:iterator>
          </tbody>
        ...
        <table width="100%">
          <tbody id="rightTable">
            ...
            <s:if test="#sts.odd == false"><tr>
              ...
              <td>${code1}</td>
              <td>${code3}</td><td><c:if test="${! empty code3}">
              <img src="${global_image_url}append.gif" style="cursor: pointer" name="a23" border="0" id="a23" onclick="addA('${code3}')" /></c:if></td>
              <td>${dcode}</td><td><c:if test="${! empty dcode}">
              <img src="${global_image_url}append.gif" style="cursor: pointer" name="d23" border="0" id="d23" onclick="addA('${dcode}')" /></c:if></td>
            </tr>
            </s:if>
            </s:iterator>
          </tbody>
