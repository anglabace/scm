...
function closeDlg(){
   $("#dlg").dialog("close");
	...
	function viewCustomer(){
		$('#new_supplier_dlg').dialog("option", "open", function() { 
        	var htmlStr = '<iframe id="iframe1" src="genscript!customerlist.action" height=350" width="600" scrolling="no" style="border:0px" frameborder="0"></iframe>';
         	$('#new_supplier_dlg').html(htmlStr);
		});
		$('#new_supplier_dlg').dialog('open');
	}
  ...
  <div class="title">${apInvoicePaymentDto.transactionTitle}</div>
      ...
      <tr>
        ...
        <td>
	        ...
	        <s:else>
	        	<input id="transactionNo" name="transactionNo" type="text"  class="NFText" value="${apInvoicePaymentDto.transactionNo}" size="40" ${apInvoicePaymentDto.transactionNo==null?'':'readonly'}/>
	        </s:else>
        </td>
      ...
      <tr>
        ...
        <td width="350">
        	...
        	<input type="button" name="Submit6" class="style_botton2" value="View Supplier" onclick="viewCustomer()" />
        </td>
...
<table  border="0" cellpadding="0" cellspacing="0" class="Customer_table">
                <tr>
                  ...
                  <td width="350">
                  <select name="orderNo" id="orderNo" style="width:220px;*width:232px;" ${apInvoicePaymentDto.status=='In Progress' && apInvoicePaymentDto.invoiceNo==null?'':'disabled'}>
                  <s:iterator value="apInvoicePaymentDto.invoiceList" >
                  	<option value="${orderNo}">${orderNo}</option>
                  </s:iterator>
                  </select>
                  <input type="button" name="Submit" class="style_botton2" value="View Order"  onclick="viewOrder()" ${apInvoicePaymentDto.status=='In Progress' && apInvoicePaymentDto.invoiceNo==null?'':'disabled'} />
                  </td>
                  <th width="130">Invoice No</th>
                  <td width="">
                  <select name="invoiceId" id="invoiceId" style="width:220px;*width:232px;" ${apInvoicePaymentDto.status=='In Progress' && apInvoicePaymentDto.invoiceNo==null?'':'disabled'}>
                  <s:iterator value="apInvoicePaymentDto.invoiceList" >
                  	<option value="${invoiceId}">${invoiceNo}</option>
                  </s:iterator>
                  </select>
                  <input type="button" name="Submit5" class="style_botton2" value="View Invoice" onclick="viewInvoice()" ${apInvoicePaymentDto.status=='In Progress' && apInvoicePaymentDto.invoiceNo==null?'':'disabled'} />
                  </td>
                </tr>
              </table>
...
function Authorize()
{
   var paymentId="${apInvoicePaymentDto.transactionId}";
   ...
   paramter=paramter+"&from=list";
    $.ajax({
	      ...
	      data:paramter,
	      success:function(data){ alert("success!");window.location.href='ap_transaction!GetList.action'; },
	      error:function(data){ alert("error!");window.location.href='ap_transaction!GetList.action';}
        }); 
