...
<script   language="JavaScript" type="text/javascript">  
	...
	function recordBatch(){
		$('#new_resource_dlg').dialog("option", "open", function() { 
        	var htmlStr = '<iframe id="paymentIframe" src="ar_invoice_payment!batchRecord.action" height="400" width="860" scrolling="auto" style="border:0px" frameborder="0"></iframe>';
...
<div class="search_box" >
  ...
  <form name="form1" id="form1" method="get" action="ar_invoice_payment!list.action">
    ...
    <th>Customer No</th>
    <td><input name="filter_EQI_custNo" type="text" class="NFText" size="20" value="${params.filter_EQI_custNo}" /></td>
    <th>Order No</th>
  ...
  <tr>
    <td colspan="6" align="center">
    	<input type="submit" name="Submit1" value="Search" class="search_input" />
     	<input type="button" name="Submit2" value="Refresh List" onclick="location.href='ar_invoice_payment!input.action'" class="search_input" />
    </td>
    </tr>
 </table>
 </form>
            ...
            <table width="1010" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr>
                <td><div class="list_box" style="width:993px;height:340px; overflow:scroll;">
                  <table width="1299" border="0" cellspacing="0" cellpadding="0" id="list_table" class="list_table">
                     <tr>
                    ...
                    <s:iterator value="arInvoicePaymentPage.result" status="stuts">
						...
						<tr>
					        ...
					        <td><a href="ar_invoice_payment!forwardRecord.action?arInvoicePaymentDto.transactionId=${transactionId }">${transactionId}</a></td>
					        <td>${custNo}</td>
					        <td>${status}</td>
					        <td>${transactionType}</td>
					        <td align="right">${symbol}${amount}</td>
					        <td align="right">${symbol}${applyAmount== null ? 0.000 : applyAmount}</td>
					        <td>${currency}</td>
					        <td >${paymentType}</td>
					        <td>${description}</td>
					        <td>${orderNo}</td>
						...
						<tr>
					        ...
					        <td class="list_td2"><a href="ar_invoice_payment!forwardRecord.action?arInvoicePaymentDto.transactionId=${transactionId }">${transactionId}</a></td>
					        <td class="list_td2">${custNo}</td>
					        <td class="list_td2">${status}</td>
					        <td class="list_td2">${transactionType}</td>
					        <td class="list_td2" align="right">${symbol}${amount}</td>
					        <td class="list_td2" align="right">${symbol}${applyAmount== null ? 0.000 : applyAmount}</td>
                ...
                <td>
                	<div class="grayr">
						<jsp:include page="/common/db_pager.jsp">
							<jsp:param value="${ctx}/ar_invoice_payment!list.action"
								name="moduleURL" />
						</jsp:include>
					</div>
                </td>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
		  <tr>
		    <td>
		    <form id="form2" name="form2" action="ar_invoice_payment!forwardRecord.action" method="post">
		    <div class="button_box">
		      <input name="Submit22" type="button" class="search_input2" onclick="window.location.href='ar_invoice_payment!forwardRecord.action?transactionId=-1'" value="Record Transaction" />
