...
<script src="${global_js_url}tools.js" type="text/javascript"></script>
	...
	function viewOrder(){
		...
		window.open("order/order!edit.action?orderNo="+orderNo+"&operation_method=view");	
	}
	...
	function selectOrder(disabled) {
	    ...
	    $('#select_order_dlg').dialog("option", "open", function() { 
	        var url = 'genscript!orderNoSearch.action?filter_EQI_custNo='+$('#custNo').val();
        	var htmlStr = '<iframe id="orderIframe" src="'+url+'" height=350" width="600" scrolling="no" style="border:0px" frameborder="0"></iframe>';
...
//applyAmount 为invoice的balance
	function setInvoiceValue(invoiceId,invoiceNo,currency,applyAmount){
		...
		if(parseFloat($('#amount').val()) < parseFloat(applyAmount)){
			$('#applyAmount').val($('#amount').val());
		}else{
			$('#applyAmount').val(applyAmount);
		}
		$('#currency').val(currency);
		var b = parseFloat($('#amount').val()) - applyAmount;//Balance可以直接取出来(实际为有效的amount – applied amount – overpayment amount)
                                                             //Over payment 如果维护, 需要减去对应数量的balance
        $('#balance').val(b); 
...
<div class="title_content">
  <div class="title">Transaction #${arInvoicePaymentDto.transactionId}</div>
</div>
        <div class="input_box">
      ...
      <tr>
        ...
        <td width="400">
	        ...
	        <s:else>
	        	<input id="transactionNo" name="transactionNo" type="text"  class="NFText" value="${arInvoicePaymentDto.transactionId}" size="40" ${arInvoicePaymentDto.transactionId==null?'':'readonly'}/>
	        </s:else>
        </td>
      ...
      <tr>
        ...
        <td>
        	...
        	<input type="button" name="Submit6" class="style_botton2" value="View Customer" onclick="viewCus2()" />
        </td>
    ...
    <div class="invoice_title">Amounts</div>
		...
		<input type="hidden" value="" name="closeInvoice"/>
	          ...
	          <table  border="0" cellpadding="0" cellspacing="0" class="Customer_table">
                <tr>
                  ...
                  <td width="400">
			          ...
			          <span class="symbol">${arInvoicePaymentDto.symbol}</span>
                  </td>
...
<table  border="0" cellpadding="0" cellspacing="0" class="Customer_table">
                <tr>
                  ...
                  <td>
                  	...
                  	<input type="button" name="Submit5" class="style_botton2" value="View Invoice" onclick="viewInvoiceInfo()" />
                  </td>
                </tr>
              </table>
       ...
       <%}else{ %>
             ...
             <div id="button_box2" class="button_box">
             <input type="button" class="search_input" value="Cancel" onclick="window.location.href='ar_approve_allocation!getList.action'" />
             </div>
      <%} %>
