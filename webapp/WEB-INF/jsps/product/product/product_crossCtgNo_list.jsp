...
function selectCatalog(){
	...
	parent.$("#crossDialog").contents().find("[name='crossDetail.rltProductId']").val(rltProductId);
...
<div  style="width:600px; height:120px; overflow:scroll;">
  <table width="583" border="0" cellpadding="0" cellspacing="0" class="list_table2">
  	<s:iterator value="productStdPriceBeanList">
	  	<tr>
	      <td width="46"><input type="radio" name="catNos" value="${catalogNo}" /></td>
	      <td width="70">&nbsp;${catalogNo}</td>
	      <td>${name}</td>
	      <td width="120">&nbsp;${leadTime}</td>
	      <td width="70"><div align="right">&nbsp;${symbol}${unitPrice}</div></td>
	    </tr>
	     <input type="hidden" name="ctgInfo_${catalogNo}" value="${name}<;>${unitPrice}<;>${productId}" />
  	</s:iterator>
 </table>
</div>
