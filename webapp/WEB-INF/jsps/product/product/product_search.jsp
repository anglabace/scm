...
<script>  
var searchType = "ProductsSearch"; 
var ProductTypeList = new Array();
<s:iterator value ="ProductTypeList" status ="itStatus">
ProductTypeList[${itStatus.count-1}] = new Array('${name}', '${name}');
</s:iterator>
...
specialAttr[0] = new Array(88,'SELECT',statusList);
specialAttr[1] = new Array(87, 'SELECT', ProductTypeList);  
...
custMySearch.getMySearch('my_search!mySearchList.action?searchType=ProductSearch');
...
function saveAdvcSrch(searchtableid, searchname){
	...
	$.ajax({
		...
		success: function(msg){
			if(msg == 'success')
			{
				alert("The search template is saved.");
			}else if(msg != '')
			{
				alert(msg);
			}
		...
		error: function (msg){
			alert("Error: Save products search failed");
		}
	});
...
function saveMySrch(searchtableid, searchname)
{
	...
	$.ajax({
		...
		error: function (msg){
			alert("Failed to save your search template. Please contact system administrator for help.");
		}
	});
...
function delCustMysrch(custmysrchid){
	...
	$.ajax({
		...
		success: function(msg){
			if(msg == 'success')
			{
				alert("The search template is deleted.");
			}else if(msg != '')
			{
				alert(msg);
			}
...
function excelSearch(){
	var url ="product/product!listExcel.action";
...
function excelASearch(searchTag){
	...
	var url ="product/product!listExcel.action"+actionUrl;
	$("#excelSearchForm").attr("action",url);
	$("#excelSearchForm").submit();
}
</script>
...
<div class="search_box">
  <div id="TabbedPanels1" class="TabbedPanels">
    ...
    <form id="mainForm" method="get" action="${ctx }/product/product.action" target="pdtServ_iframe">
    <input type="hidden" name="search_type" value="search" />
    <br />
    <table border="0">
    <tr>
    	<td width="106" align="right">Product Catalog No</td>
        <td width="134"><input name="filter_LIKES_catalogNo2" id="catNo" class="NFText" size="20" type="text" /></td>
    ...
    <table width="725">
      ...
      <form method="post" id="advancedSearchForm" action="${ctx }/product/product.action" _action="product/product.action" target="pdtServ_iframe">
	  ...
	  <table width="725">
      	<tr><th>
			<input type="button"  value="Search" id="advancedSearch" class="search_input" />
            
            &nbsp;&nbsp;<input type="button" value="Export" class="search_input" onclick="excelASearch('advancedSearch');"/>
		</th></tr>
      </table>
      </form>
      </div>
      ...
      <div style="display:none" class="TabbedPanelsContent">
      <form method="post" id="mySearchForm" action="${ctx }/product/product.action" _action="product/product.action" target="pdtServ_iframe">
...
<div class="input_box">
		<div class="content_box">
			<iframe src="${ctx }/product/product.action" name="pdtServ_iframe" id="pdtServ_iframe" width="1010" height="560" frameborder="0" scrolling="no"></iframe>
		</div>
  </div>	
</div>	
