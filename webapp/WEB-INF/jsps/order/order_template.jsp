		...
		$(function(){
			...
			$("#del_tmpl_img").click( function() {
			   	...
			   	if (! confirm("Are you sure to delete ?")) {
			   	 ...
			   	 $.ajax( {
				       type: "post",
				        url: "order_extra!delTemplate.action",
			...
			$("#saveTmplBtn").click( function(event) {	
				 ...
				 if ($("#maxTemplate").val()<=$("#tmpl_tbl tr").length) {
				    if ($("#overrideFlag").val()=='N') {
				      alert("Your order template's count is more than the maxium " + $("#maxTemplate").val() + "; Please delete one order template first. ");
				      return false;
				    }
				 }	
				 $.ajax( {
				       type: "post",
				        url: "order_extra!saveTemplate.action",
