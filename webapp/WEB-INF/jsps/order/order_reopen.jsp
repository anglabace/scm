...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<title>Reopen Order Information</title>
...
$(document).ready(function() {
	$("#reopenBtn").click( function(){
			...
			$.ajax({
				   type: "POST",
				   url: "order_extra!reopen.action",
				   data: "sessOrderNo=${sessOrderNo}&orderStatus="+status+"&statusReason="+statusReason+"&statusNote="+statusNote,
				   success:function(data) {
					   if ("successful" == data) {
							alert("The order is reopened .");
							...
							//window.location = globalUrl + orderquoteObj.type +"/"+orderquoteObj.type+"/showEditFormAct?"+noName+"="+data+"&custNo="+custNo+"&fullName="+fullName+"&defaultTab="+defaultTab;
					...
					error: function(msg) {
			            alert("Failed to reopen the order.");
		            },
					async: false
		    });
 	});
