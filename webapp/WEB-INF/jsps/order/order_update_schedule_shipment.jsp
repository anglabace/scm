...
$(document).ready(function(){
	$("#updateOrderItemScheduleDeliveryBtn").click(function(){
        ...
        var paramStr = "";
        if($("#update_reason_sel").val() != undefined){
            paramStr = "&update_reason_sel="+$("#update_reason_sel").val();
        }
		reason = encodeURIComponent(reason);
		$.ajax({
			type:"post",
			url:orderquoteObj.url("updateItemScheduleShipment"),
			data:"&itemId="+itemId+"&newScheduleShipment="+newScheduleShipment+"&reason="+reason+paramStr,
			dataType:"json",
			success:function(data){
				...
				parent.$('#updateOrderScheduleDeliveryDialog').dialog('close');
			},
			error:function(){
...
<title>Update Order Schedule Delivery</title>
</head>
<body>
<table id="updateOrderScheduleDeliveryTable"  border="0" align="center" cellpadding="0" cellspacing="0" class="General_table"  style="margin:10px auto 0px auto;">
	  <tr>
	    <th width="150" valign="top"><span class="important">*</span>New Schedule Delivery:</th>
	    <td  >
	      <input type="text" id="newScheduleShipment" name="newScheduleShipment" value="${orderItem.shipSchedule}" class="NFText" size="34"/>
	      <input type="button" id="calculate" name="calculate" class="style_botton"  value="Calculate" />
	    </td>
	  </tr>
    <s:if test="updateReasonSel != null">
        <tr>
            <th height="26">
                Update Reason
            </th>
	  ...
	  <tr>
	    <td colspan="2">
	    	<div align="center"><br />
		       <input type="button" class="style_botton" value="Confirm" id="updateOrderItemScheduleDeliveryBtn" />
		       <input  type="button" name="Cancel" value="Cancel" class="style_botton" onclick="parent.$('#updateOrderScheduleDeliveryDialog').dialog('close');" />
	    	</div>
	    </td>
	  </tr>
	</table>
