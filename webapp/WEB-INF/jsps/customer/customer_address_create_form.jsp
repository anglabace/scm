...
<script language="JavaScript" src="${global_js_url}scm/customerAddr.js"
...
<script>
    $().ready(function() {
        ...
        $("#addr_add_form").validate({
            ...
            rules: {
                ...
                "addrData.addrLine2": {  maxlength : 50},
                "addrData.addrLine3": { maxlength : 50},
            ...
            messages: {
                "addrData.firstName":{required: "Please enter your firstname",
                ...
                "addrData.country": {required:"Country is required"} ,
                "addrData.zipCode": {required: "Please enter the zip.",
    ...
    $(function () {
        $('#orgAddDialogTrigger')
            ...
            dataHolderWin.jQuery.data(dataHolderWin.document.body, 'isGetAddr', "true");
            dataHolderWin.jQuery.data(dataHolderWin.document.body, 'orgLoc', self);
            dataHolderWin.jQuery.data(dataHolderWin.document.body, 'orgAddr', "address_1:address_2:address_3");
...
<script>
    ...
    function add_address(bt_obj, form_name, action_name) {
        ...
        if ($("input[name='validation_date']:checked").val() == 1) {
            ...
            if ($("#effTo").attr('value') == "") {
                alert("Please input validation to date");
                $("#effTo").focus();
                return false;
            }
        ...
        //	var moreData = addrIframe.contentWindow.document.getElementById(defaultTmpId+'_allData').value;
        ...
        var options = {
            success:function(data) {
                ...
                parent.document.getElementById('addrIframe').src = "cust_address!list.action?custNo=${custNo}&sessCustNo=${sessCustNo}";
                parent.$('<s:if test="addrData.addrId">#editCustomerAddrDialog</s:if><s:else>#addCustomerAddrDialog</s:else>').dialog('close');
            },
            error: function() {
                alert('error...');
                bt_obj.value = bt_val;
    ...
    $(document).ready(function() {
        $("#status").click(function () {
            var defaultFlag = $("#defaultFlag").attr("checked");
            if (defaultFlag) {
                $("#status").attr("checked", true);
                return false;
            }
        });
        //$("#nameSfx option:first").attr("selected", true);
        //$("#namePfx option:first").attr("selected", true);
        //slt.options[slt.selectedIndex].get(0)='';
    });
...
<body>
	<form method="get" action="cust_address!save.action" id="addr_add_form">

		<input type="hidden" id="sessCustNo" name="sessCustNo"
			value="${sessCustNo}" /> <input type="hidden" name="custNo"
			value="${custNo}" /> <input type="hidden" value="${defaultFlags}"
			name='defaultFlags' id="defaultFlags" /> <input type="hidden"
			value="${addrData.addrId}" id='defaultTmpId' name="defaultTmpId" />
		<input type="hidden" value="${addrData.addrId}" id='addrId'
			name="addrData.addrId" /> <input type="hidden"
			value="${addrData.operateType}" id="operateType"
			name="addrData.operateType" />
		<table border="0" align="center" cellpadding="0" cellspacing="0"
			...
			<tr>
				...
				<td><s:select id="addrType" name="addrData.addrType"
						...
						listValue="value" cssStyle="width:225px" value="addrData.addrType"
			...
			<tr>
				...
				<td colspan="3" valign="top"><input type="button"
					...
					onclick="select_address('${addrData.addrId}', '${custNo}');"
					class="style_botton2" /></td>
			</tr>
			<tr>
				<td align="right">Customer #</td>
				<td><input name="" disabled="disabled" readonly
					value="${custNo}" type="text" class="NFText3" size="38" /></td>
				<td align="right">Alternate #</td>
				<td colspan="3"><input name="alternate" disabled="disabled"
					value="${customerDetail.altNo}" type="text" id="alternate"
					size="40" class="NFText3" readonly /></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td colspan="3">
					<%-- <input name="addrData.orgId"
							value="${addrData.organization.orgId}" type="hidden" id="orgId" />
										value="${addrData.orgId}"
					type="hidden" id="orgId" /> --%> <input name="addrData.orgName"
					value="${addrData.orgName}" readonly="readonly" type="text"
					id="orgName" size="40" class="NFText" /> <img
			...
			<tr>
				...
				<td>
					<div align="right">
						<s:if test='addrData.busPhone!=""'>
							<input name="addrData.busPhone" type="text"
								value="${addrData.busPhone}" id="busPhone" size="20"
								class="NFText" />
						</s:if>
						<s:else>
							<input name="addrData.busPhone" type="text" value=""
								id="busPhone" size="20" class="NFText" />
						</s:else>
						Ext <input name="addrData.busPhoneExt"
							value="${addrData.busPhoneExt}" type="text" size="8"
							id="busPhoneExt" class="NFText" />
					</div>
				</td>
			...
			<tr>
				...
				<td colspan="5"><input name="addrData.addrClass" type="radio"
					value="Commercial" checked="checked" /> Commercial <input
					type="radio" name="addrData.addrClass" value="Residential"
					<s:if
                    test='addrData.addrClass=="Residential"'>checked="checked"</s:if> />
					Residential</td>
			</tr>
			<tr>
				<td valign="top">Validation Date</td>
				<td colspan="5"><input name="validation_date" type="radio"
					value="0" checked onclick="aCheck1()" /> No date restrictions <input
					type="radio" name="validation_date" value="1" onclick="bCheck2()" />
					Periodical
				...
				<td colspan="6" valign="top">
					<div align="center">
						<br /> <input type="button"
							onclick="add_address(this, 'addr_add_form', '${global_url}cust_address!save.action');"
							id="addr_add_button"
							value="<s:if test='addrData!=null&&addrData.addrId!=null'> Edit  </s:if><s:else>Add </s:else>"
							class="style_botton" /> <input type="button" value="Cancel"
							onclick="parent.$('<s:if test="addrData.addrId">#editCustomerAddrDialog</s:if><s:else>#addCustomerAddrDialog</s:else>').dialog('close');"
							class="style_botton" />
					</div>
				</td>
			</tr>
    ...
    <s:if test='addrData.effFrom!=null'>
            set_checked("validation_date", "1");
    </s:if>
    <s:else>
    set_checked("validation_date", "0");
    </s:else>
    set_checked("addrClass", "${addrData.addrClass}");
</script>
	<script type="text/javascript"
		src="${global_js_url}scm/gsCountryState.js"></script>
