...
function add_file(){ 
	...
	var height = parent.$('#note_dlg').dialog('option','height');
	parent.$('#note_dlg').dialog('option','height',height+10);
}
...
function update_note(op_type){
	...
	var action = 'organization!saveInstruction.action';
	...
	var options = {
		...
		success:function(data) {
			...
			if (pos == 0){
				//insert
				slt.options.add(new Option(noteType+" - instr - org", note_id, false, true));
				...
				slt.options.add(new Option(noteType+" - instr - org", note_id, false, true), pos);
			}
