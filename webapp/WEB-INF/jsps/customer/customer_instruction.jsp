...
function update_note(op_type){
	...
	var action = 'customer!saveInstruction.action';
	...
	var options = {
		...
		success:function(data) {
			...
			data = data.replace(/&lt;\/pre&gt;/gi, "");
			var note_id = data;
			var slt = parent.document.getElementById('instruction_slt'); 
			...
			//	slt.options.add(new Option(noteType+"#"+slt.options.length, note_id, false, true));
				slt.options.add(new Option(noteType+" - instr - account", note_id, false, true));
				parent.$("#instruction_btn").val("View/Edit");
			}else{
				//edit
				...
				slt.options.add(new Option(noteType+" - instr - account", note_id, false, true), pos);
			}
			parent.$('#accountingInstructionDialog').dialog('close'); 
		}, 
		error: function(){
			alert('error...'); 
		}, 
	  ...
	  <tr>
		...
		<td width="180">
			<s:select name="noteDTO.type" id="instruction_type" cssStyle="width:170px;" value="noteDTO.type"
                      list="#{'ACCOUNTING':'Accounting Notes', 'PRODUCTION':'Production Notes', 'SALES_NOTES':'Sales Notes', 'SHIPMENT':'Shipment Notes'}">
			</s:select>
		</td>
		<td>
			&nbsp;
	 ...
	 <table width="600"  border="0" cellpadding="0" cellspacing="0" class="General_table"> 
		<tr>
		  <td colspan="2">
			...
			&nbsp;&nbsp;
			<input type="submit" value="Cancel"  class="style_botton" onclick="javascript:parent.$('#accountingInstructionDialog').dialog('close');parent.$('#accountingInstructionDialog2').dialog('close');" />
			</div>
		 </td>
		  </tr>
	</table>
