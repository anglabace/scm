...
function fzCategoryNo(uId){
	...
	var servCategoryList = parent.$("#servCategoryId").val();
	var servCategory =  servCategoryList.split("<;;>");
	for(var i = 0;i<servCategory.length;i++){
		if(ctgId==servCategory[i]){
			alert(ctgNoS+" has been added.");return;
		}
	}
	var obj_subCategory = parent.$("#catalog_service_Category_iframe").contents();
	...
	obj_subCategory.find("#ctgMby").val($("#ctgMby"+uId).val());
	obj_subCategory.find("#ctgCdate").val($("#ctgCdate"+uId).val());
	obj_subCategory.find("#ctgCby").val($("#ctgCby"+uId).val());
	obj_subCategory.find("#ctgBuss").val($("#ctgBuss"+uId).val());
	parent.$('#chooseCategoryDialog').dialog('close');
	parent.$('#chooseCategoryDialog').dialog('destory');
}
...
<body>
	<b>Choose Business Unit: Business Unit List</b><br />
	<form id="mainForm" action="${ctx}/serv/service_category!ctlgChooseCatList.action" method="get">
	<table width="300" border="0" cellpadding="0" cellspacing="0" class="list_table">
          <tr>
            <th width = "150">Business Unit No</th>
            <th width = "150">Business Unit Name</th>
            </tr>
        </table>
    ...
    <td width="150">
        ...
        <input type="hidden" id="ctgBuss${categoryId}" value="${businessDivision}" />
    ...
    <jsp:include page="/common/db_pager.jsp">
		<jsp:param value="${ctx}/serv/service_category!ctlgChooseCatList.action" name="moduleURL"/>
	</jsp:include>
	</div>
