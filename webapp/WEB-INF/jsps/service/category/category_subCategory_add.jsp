...
$().ready(function(){
	...
	$("#catAddTrigger").click(function(){
		parent.$('#chooseSubCategoryDialog').dialog("option", "open",function(){
			...
			var htmlStr = '<iframe id="chooseCategory_iframe" src="${ctx}/serv/service_category!cateChooseSubCatList.action?categoryLevel=${categoryLevel}&sessionCategoryId=${sessionCategoryId}&categoryId=${categoryId}" height="400" width="320" scrolling="no" style="border:0px" frameborder="0"></iframe>';
...
function saveSubCategory(){
	...
	if(ctgNo.length>60){
		alert("The length of the category No is out of maximum limit 60.");
		$("input[name='ctgNo']").focus();
		return;
	}
	...
	$.ajax({
		type:"POST",
		url:"serv/service_category!addSubServiceCategory.action?serviceCategoryKey="+ctgId,
		...
		success: function(msg){
			if(msg=="success"){
				...
				var https = '<a href="${ctx }/serv/service_category!input.action?categoryId='+ctgId+'&callBackName=categoryCreationForm&operation_method=edit" target="_parent">'
