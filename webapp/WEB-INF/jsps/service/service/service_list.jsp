...
$(document).ready(function(){  
    ...
    $("#saveApprovedStatusTrigger").click(function(){
		...
		if(approvedReason==''||approvedReason==null){
			alert("Please enter the reason to continue your operation.");return;
		}
		 if(!confirm("Are you sure to delete?")){
	            return;
	        }
        $.ajax({
			url:"serv/serv!delServiceListApproved.action",
			type:"get",
			...
			success:function(data){
				if(hasException(data)){
					...
					window.location.href="serv/serv!search.action";
				}
			},
			error:function(data){
				alert("System error! Please contact system administrator for help.");
  ...
  <div class="list_box" style="height:320px; width:993px;">
      ...
      <tr>
        ...
        <s:if test="status==\"ACTIVE\"">
					...
					<s:iterator value="statusOfapproved" id='number'>
        				<s:if test="#number==serviceId">
        				    <s:set name="approvedServiceId" value="#number"></s:set>
        					<input type="checkbox" value="${serviceId}" name="pdtServSeq" id="pdtServSeq" disabled="disabled"/>
        				</s:if>
        			</s:iterator>
        		    <s:if test="#approvedServiceId!=serviceId">
        		    	<input type="checkbox" value="${serviceId}" name="pdtServSeq" id="pdtServSeq" />
        		    	 <s:set name="approvedServiceId" value="null"></s:set>
        		    </s:if>
        </s:if>
        <s:else>
        	<input type="checkbox" value="${serviceId}" name="pdtServSeq" id="pdtServSeq" disabled="disabled"/>
        ...
        <td width="70" style="word-break:break-all;word-wrap:break-word;width:70">
        	<a href="${ctx}/serv/serv!input.action?id=${serviceId}&operation_method=edit" target="_parent">
            	${catalogNo}&nbsp;
            </a>
        </td>
	...
	<div class="grayr">
		<jsp:include page="/common/db_pager.jsp">
			<jsp:param value="${ctx }/serv/serv.action" name="moduleURL"/>
		</jsp:include>
	</div>
  </div>
  </div>
    <div  class="button_box">
 &nbsp;&nbsp;<input type="button" value="New" class="search_input" onclick="parent.location.href='${ctx}/serv/serv!input.action';" />
