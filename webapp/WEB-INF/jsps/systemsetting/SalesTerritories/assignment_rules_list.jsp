	    ...
	    $("#check_del").click(function(){
			 if ($(":checkbox:gt(0):checked").length < 1) {
			      alert('Please select one to continue your operation!');
					...
					$.ajax({
						type: "POST",
						url: "assignment_rules!delete.action",
...
<table width="1010" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
	...
	<div class="list_box">
      ...
      <s:if test="salesTerritoryAssignRulesPage!=null&&salesTerritoryAssignRulesPage.result!=null">
      <s:iterator value="salesTerritoryAssignRulesPage.result" status="st">
      <s:if test="#st.odd">
	      <tr>
	      	 <td width="46"><input type="checkbox" value="${ruleId}" id="${ruleId}" onclick="choice(this);" name=delRouteId/></td>
	      	 <td align="center" width="60">${ruleId}</td>
	         <td width="258"><a href="assignment_rules!load.action?ruleId=<s:property value='ruleId'/>" target="_parent">${ruleName}</a></td>
	         <td width="209">${status}</td>
	         <td>${description}</td>
	      </tr>
      </s:if>
      <s:else>
        ...
        <td align="center" class="list_td2">${ruleId}</td>
        <td class="list_td2"><a href="assignment_rules!load.action?ruleId=<s:property value='ruleId'/>" target="_parent">${ruleName}</a></td>
  ...
  <tr>
	  ...
	  <jsp:include page="/common/db_pager.jsp">
			<jsp:param value="${ctx}/assignment_rules!list.action" name="moduleURL" />
	  </jsp:include>
	  </div>
	  </td>
  </tr>
