...
function addResource(){
     ...
     parent.$("#salesId").val(array[0]);
 	 ...
 	 parent.$("#description").val(array[4]);
...
<body>
<s:form action="sales_group!selectResource"  name="docTempSearch" method="get" onsubmit="return check();">
				...
				<td colspan="4">
					<div style="width: 830px; height: 210px; overflow: scroll;">
						<table width="820" border="0" cellpadding="0" cellspacing="0"
							...
							<s:iterator value="salesRepPage.result" status="st">
					      	<s:if test="#st.odd">
					      	<tr>
					          <td width="46"><input type="radio" value="${salesId}::${resourceName}::${deptId}::${status}::${description}::${modifyDate}::${modifiedName}" id="${salesId}" name="salesId"/></td>
					      	...
					      	<s:else>
					      	<tr>
					          <td class="list_td2"><input type="radio" value="${salesId}::${resourceName}::${deptId}::${status}::${description}::${modifyDate}::${modifiedName}" id="${salesId}" name="salesId"/></td>
					...
					<div class="grayr">
						<jsp:include page="/common/db_pager.jsp">
							<jsp:param value="${ctx}/sales_group!selectResource.action" name="moduleURL" />
						</jsp:include>
					</div>
				</td>
			</tr>
