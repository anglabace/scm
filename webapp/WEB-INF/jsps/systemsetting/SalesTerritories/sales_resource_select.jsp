...
function addUser(){
	 ...
	 var value = $("#resultTable").find("[name='userId']:checked").val();
     var array = value.split("::");
     parent.$("#salesId").val(array[0]);
     parent.$("#resourceName").val(array[1]);
     parent.$("#deptId").val(array[2]);
     parent.$("#deptName").val(array[3]); 
...
function check() {
	  ...
	  if($("#filter_EQI_userId").val()!=""&&isNaN($("#filter_EQI_userId").val())) {
...
<body>
<s:form action="sales_resource!selectUser"  name="docTempSearch" method="get" onsubmit="return check();">
<table border="0" cellpadding="0" cellspacing="0"
				class="General_table" align="center">
        <tr>
         <td>User ID</td>
          <td>
          	<input  id="filter_EQI_userId" name="filter_EQI_userId" type="text" class="NFText" size="20" >
          </td>
          <td>User Name</td>
          <td>
				...
				<td colspan="4">
					<div style="width: 657px; height: 210px; overflow: scroll;">
						<table width="640" border="0" cellpadding="0" cellspacing="0"
							...
							<s:iterator value="usersPage.result" status="st">
					      	<s:if test="#st.odd">
					      	<tr>
					          <td width="46"><input type="radio" value="${userId}::${loginName}::${deptId}::${deptName}" id="${userId}" name="userId"/></td>
					      	...
					      	<s:else>
					      	<tr>
					          <td class="list_td2"><input type="radio" value="${userId}::${loginName}::${deptId}::${deptName}" id="${userId}" name="userId"/></td>
					...
					<div class="grayr">
						<jsp:include page="/common/db_pager.jsp">
							<jsp:param value="${ctx}/sales_resource!selectUser.action" name="moduleURL" />
						</jsp:include>
					</div>
				</td>
			</tr>
