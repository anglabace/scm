...
this.createAJAX();
	...
	var tabView_maxNumberOfTabs = 12;	// Maximum number of tabs
	...
	function showTab(parentId,tabIndex)
	{
		...
		if(activeTabIndex[parentId]>=0){
			...
			img.src = 'images/tab_inactive2.gif';
		...
		img.src = 'images/tab_active2.gif';
	...
	function changeTabClass(tabIndex, domain, className)
	{
//		var parentId = this.parentNode.parentNode.id;
//		var parentId_div = parentId + "_";
//		var thisObj = document.getElementById('tabTab'+ parentId_div +tabIndex);
		if(domain == 'document')
		var thisObj = document.getElementById('tabTabdhtmlgoodies_tabView1_'+tabIndex);
		else if(domain == 'parent')
		var thisObj = parent.document.getElementById('tabTabdhtmlgoodies_tabView1_'+tabIndex);
		if(thisObj)
		thisObj.className=className;
	} 
	
	function changeTabClass2(tabIndex, domain, className)
	{
//		var parentId = this.parentNode.parentNode.id;
//		var parentId_div = parentId + "_";
//		var thisObj = document.getElementById('tabTab'+ parentId_div +tabIndex);
		if(domain == 'document')
		var thisObj = document.getElementById('tabTabdhtmlgoodies_tabView2_'+tabIndex);
		else if(domain == 'parent')
		var thisObj = parent.document.getElementById('tabTabdhtmlgoodies_tabView2_'+tabIndex);
		if(thisObj)
		thisObj.className=className;
	} 

	//use reference: system settings/shipping methods/charge customization/basic charge
	function changeTabClassWithPrefix(tabIndex, domain, className, prefix)
	{
		if(domain == 'document'){
			var thisObj = document.getElementById(prefix+tabIndex);
		}else if(domain == 'parent'){
			var thisObj = parent.document.getElementById(prefix+tabIndex);
		}
		if(thisObj){ 
			if (className == 'tabDisable'){
				thisObj.onclick = null;
			}else{
				thisObj.onclick = tabClick; 
			}
			thisObj.className=className;
		}
	} 
	
	function getTabByIndex(tabIndex, domain){
		if(domain == 'parent'){
			var thisObj = parent.document.getElementById('tabTabdhtmlgoodies_tabView1_'+tabIndex);
		}else{
			var thisObj = document.getElementById('tabTabdhtmlgoodies_tabView1_'+tabIndex);
	...
	function disableTabByTitle(tabTitle){
		var tabIndex = getTabIndexByTitle(tabTitle); 
		if(tabIndex == -1){
			return;
		}
		var index = tabIndex[1];
   		var thisTab = getTabByIndex(index, "document");
   		thisTab.onclick = null;
   		thisTab.className = 'tabDisable';
	}
	
	function enableTabByTitle(tabTitle){
		var tabIndex = getTabIndexByTitle(tabTitle); 
		if(tabIndex == -1){
			return;
		}
		var index = tabIndex[1];
   		var thisTab = getTabByIndex(index, "document");
   		thisTab.onclick = tabClick;
   		var parentId = thisTab.parentNode.parentNode.id;
   		if(activeTabIndex[parentId] == index){
   			thisTab.className='tabActive';
   		}else{
   			thisTab.className='tabInactive';
   		}
	}
	 ...
	 * @return
	 */
	function getTabIndexBy(tabTitle){
		var tabIndex = getTabIndexByTitle(tabTitle); 
		if(tabIndex == -1){
			return tabIndex;
		}
	...
	function initTabs(mainContainerID,tabTitles,activeTab,width,height,closeButtonArray,additionalTab)
	{
		...
		for(var no=0;no<tabTitles.length;no++){
			...
			/**
			> * {2010-01-19} - {��ΰ}: {Ϊ���޸�user_managerment��role_managerment����¼������span id Ϊ�˶�λspan�ĵ���¼�?
			> */
			span.id = tabTitles[no];
	...
	function initTabs_new(newclass,mainContainerID,tabTitles,activeTab,width,height,closeButtonArray,additionalTab)
	{
		...
		var divCounter = 0;
		for(var no=0;no<tabs.length;no++){
			if((tabs[no].className=='dhtmlgoodies_aTab_new'+newclass) && tabs[no].parentNode.id == mainContainerID){
				if(height.length>0)tabs[no].style.height = height;
				tabs[no].style.display='none';
				tabs[no].id = 'tabView' + mainContainerID + "_" + divCounter;
				divCounter++;
