...
dhtmlXTreeObject.prototype.loadXML=function(file, afterCall , async )
{
	...
	this.XMLLoader=new dtmlXMLLoaderObject(this._parseXMLTree,this,async,this.no_cashe);
	if (afterCall)
	{
		this.XMLLoader.waitCall=afterCall;
	}
...
dhtmlXTreeObject.prototype._attachChildNode=function(parentObject,itemId,itemText,itemActionHandler,image1,image2,image3,optionStr,childs,beforeNode,afterNode){if (beforeNode && beforeNode.parentObject)parentObject=beforeNode.parentObject;if (((parentObject.XMLload==0)&&(this.XMLsource))&&(!this.XMLloadingWarning))
...
dhtmlXTreeObject.prototype.getUserData=function(itemId,name){
	var sNode=this._globalIdStorageFind(itemId,0,true);
	if (!sNode)return;return sNode.userData["t_"+name]
};
...
dhtmlXTreeObject.prototype.getItemColor=function(itemId)
 ...
 {if (defaultColor)temp.span.style.color=defaultColor};if (selectedColor)temp.scolor=selectedColor;if (defaultColor)temp.acolor=defaultColor}};
...
dhtmlXTreeObject.prototype.refreshItem=function(itemId){if (!itemId)itemId=this.rootId;var temp=this._globalIdStorageFind(itemId);this.deleteChildItems(itemId);this._loadDynXML(itemId)};dhtmlXTreeObject.prototype.setItemImage2=function(itemId, image1,image2,image3){var temp=this._globalIdStorageFind(itemId);if (!temp)return 0;temp.images[1]=image2;temp.images[2]=image3;temp.images[0]=image1;this._correctPlus(temp)};dhtmlXTreeObject.prototype.setItemImage=function(itemId,image1,image2)
 ...
 if (zb)z+=this.dlmtr+zb}};return z};
...
dhtmlXTreeObject.prototype._getAllSubItemsProperty =function(itemId,z,node)
{
	if (node)
		temp=node;
	else 
	...
	{
		if (!z)
			z=temp.childNodes[i].id +"@@"+this.getItemText( temp.childNodes[i].id ) +"@@"+this.getUserData( temp.childNodes[i].id , "privilegeType" ) +"@@"+this.getUserData( temp.childNodes[i].id , "parentCode" ) ;
			
		else
			z+=this.dlmtr+temp.childNodes[i].id + "@@"+this.getItemText( temp.childNodes[i].id ) +"@@"+this.getUserData( temp.childNodes[i].id , "privilegeType" ) +"@@"+this.getUserData( temp.childNodes[i].id , "parentCode" ) ;
		var zb=this._getAllSubItemsProperty(0,z,temp.childNodes[i])
		if (zb)
			z+=this.dlmtr+zb
	};
	return z
};
dhtmlXTreeObject.prototype.selectItem=function(itemId,mode,preserve){mode=convertStringToBoolean(mode);var temp=this._globalIdStorageFind(itemId);if ((!temp)||(!temp.parentObject)) return 0;if (this.XMLloadingWarning)temp.parentObject.openMe=1;else
...
dhtmlXTreeObject.prototype.getAllSubItemsProperty =function(itemId)
{
	return this._getAllSubItemsProperty(itemId)
};

dhtmlXTreeObject.prototype.getAllChildless =function(){return this._getAllScraggyItems(this.htmlNode)};dhtmlXTreeObject.prototype.getAllLeafs=dhtmlXTreeObject.prototype.getAllChildless;dhtmlXTreeObject.prototype._getAllScraggyItems =function(node)
 {var z="";for (var i=0;i<node.childsCount;i++){if ((node.childNodes[i].unParsed)||(node.childNodes[i].childsCount>0))
 {if (node.childNodes[i].unParsed)var zb=this._getAllScraggyItemsXML(node.childNodes[i].unParsed,1);else
 var zb=this._getAllScraggyItems(node.childNodes[i])

 if (zb)if (z)z+=this.dlmtr+zb;else z=zb}else
 if (!z)z=node.childNodes[i].id;else z+=this.dlmtr+node.childNodes[i].id};return z};
