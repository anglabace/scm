...
dhtmlXTreeObject.prototype.enableItemEditor=function(mode){
 ...
 dhtmlXTreeObject.prototype.setOnEditHandler=function(func){this.attachEvent("onEdit",func)};dhtmlXTreeObject.prototype.
 
setEditStartAction=
function(click_IE, dblclick)
{
	...
	if (this._editCell)
	{
		...
		if (this._editCell.id!=a)
		{
			...
			if (editText===true)
				...
				editText=this._editCell._oldValue;
...
dhtmlXTreeObject.prototype.stopEdit=function()
{
 ...
 dhtmlXTreeObject.prototype._editItem=function(id,tree){
	if (this._eItEd)
	{
		...
		var self=this;temp.span.childNodes[0].onkeydown=function(e)
		{
			if (!e)e=window.event;
			if (e.keyCode==13)
			{
			...
			{
				self._editCell.span.childNodes[0].value=self._editCell._oldValue;self._stopEdit(window.undefined)
			};
			(e||event).cancelBubble=true
		};
