...
$(function(){
			...
			success: function(data, textStatus){
				if(hasException(data))
				{
					...
					if(data.back=='product'){
						location.href = "product/product_category!input.action?dodo=first&callBackName=categoryCreationForm&categoryId="+data.id+"&defaultTab="+defaultTab;
					}else{
	...
	$("#categoryTopForm").validate({
		...
		rules: {
    		"productCategoryDTO.categoryNo": {required:true, minlength:1, maxlength:60},
    		"productCategoryDTO.name": {required:true, minlength:1, maxlength:50}
		},
		messages: {
			"productCategoryDTO.categoryNo": {
				required: "Please enter the Catalog Id",
				maxlength: "The length of the Category No  is out of maximum limit - max length 60."
			},
	...
	$("#saveApprovedTrigger").click(function(){
		...
		$.ajax({
			url:"product/product_category!productCategoryApprovedSaveSession.action",
			type:"get",
